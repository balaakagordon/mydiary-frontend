...
describe("entries actions", () => {
  ...
  it("creates GET_ENTRY when a user logs retrieves an entry", () => {
    ...
    fetchMock.get(`${baseurl}/api/v1/entries/${entryId}`, responseData);
    ...
    store.dispatch(getEntry(entryId)).then(() => {
      expect(store.getActions()).toEqual(expectedActions);
    });
  });
  ...
  it("creates UPDATE_ENTRY when a user submits an entry", () => {
    ...
    fetchMock.put(`${baseurl}/api/v1/entries/${entryId}`, responseData);
    ...
    store.dispatch(updateEntry(entryId)).then(() => {
      expect(store.getActions()).toEqual(expectedActions);
    });
  });
});
